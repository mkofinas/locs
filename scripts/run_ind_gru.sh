...
do
  ...
  CUDA_VISIBLE_DEVICES=$GPU python -u locs/experiments/ind_experiment.py --gpu --mode train --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS
  EVAL_ARGS="--report_error_norm --train_data_len 100 --batch_size 1 --max_burn_in_count 5 --verbose --final_test --test_short_sequences --error_out_name test_prediction_errors_shortseq_burnin5.npy --load_best_model"
  CUDA_VISIBLE_DEVICES=$GPU python -u locs/experiments/ind_experiment.py --gpu --mode eval --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $EVAL_ARGS
done
