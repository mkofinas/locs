...
from locs.models.model_utils import RefNRIMLP, encode_onehot
...
class DNRI(nn.Module):
    ...
    def multisample_single_step_forward(self, inputs, decoder_hidden,
                                        edge_logits, hard_sample, num_samples):
        if isinstance(inputs, list):
            old_shape = tuple([num_samples]) + edge_logits[0].shape
            edges = model_utils.gumbel_softmax(
                torch.stack(edge_logits).reshape(-1, self.num_edge_types),
                tau=self.gumbel_temp,
                hard=hard_sample).view(old_shape)
        else:
            old_shape = tuple([num_samples]) + edge_logits.shape
            edges = model_utils.gumbel_softmax(
                edge_logits.expand(old_shape).reshape(-1, self.num_edge_types),
                tau=self.gumbel_temp,
                hard=hard_sample).view(old_shape)
        predictions = [None for _ in range(num_samples)]
        decoder_hidden_out = [None for _ in range(num_samples)]
        for i in range(num_samples):
            if isinstance(inputs, list):
                predictions[i], decoder_hidden_out[i] = self.decoder(inputs[i], decoder_hidden[i], edges[i])
            else:
                predictions[i], decoder_hidden_out[i] = self.decoder(inputs, decoder_hidden, edges[i])
        return predictions, decoder_hidden_out, edges
