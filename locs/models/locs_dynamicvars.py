...
from locs.models.anisotropic_filter import AnisotropicEdgeFilter
from locs.utils.rotation_utilities import (
    canonicalize_inputs, create_edge_attr_pos_vel)
...
class LoCSDynamicVars(nn.Module):
    def __init__(self, params):
        super().__init__()
        # Model Params
        self.encoder = DynamicVarsEncoder(params)
...
class DynamicVarsEncoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.num_relative_features = 7
        self.num_pos_features = 3
        self._anisotropic = not params.get('isotropic', False)
        if self._anisotropic:
            ...
            self.edge_filter = RefNRIMLP(
                self.num_relative_features+inp_size, hidden_size, hidden_size,
                dropout, no_bn=no_bn)
    ...
    def _edge_filter(self, edge_attr, edge_pos):
        return (self.edge_filter(edge_attr, edge_pos) if self._anisotropic
                else self.edge_filter(edge_attr))
    ...
    def compute_feat_transform(self, inputs, node_masks):
        ...
        for inputs, node_masks in zip(inp_list, node_masks_list):
            ...
            edge_attr = self._edge_filter(edge_attr, edge_pos)
            x_skip = edge_attr
...
class DynamicVarsDecoder(nn.Module):
    def __init__(self, params):
        ...
        self.present_gnn = params.get('present_gnn', False)
        if self.present_gnn:
            ...
            if self._anisotropic:
                ...
                self.edge_filter = nn.ModuleList([
                    nn.Sequential(
                        nn.Linear(self.num_relative_features+input_size, n_hid),
                        nn.ReLU(),
            ...
            self._edge_filter = [
                partial(self._edge_filter_i, i) for i in range(edge_types)]
            print(self._edge_filter)
    ...
    def _edge_filter_i(self, i, edge_attr, edge_pos):
        return (self.edge_filter[i](edge_attr, edge_pos) if self._anisotropic
                else self.edge_filter[i](edge_attr))
        ...
        if num_vars > 1:
            ...
            if self.present_gnn:
                ...
                for i in range(start_idx, self.edge_types):
                    msg = self._edge_filter[i](edge_attr, edge_pos)
                    msg = msg * edges[:, :, i:i + 1]
                    present_msgs += msg
