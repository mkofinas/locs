...
import locs.datasets.utils.acm_parser as acm_parser
...
class CmuMotionData(Dataset):
    def __init__(self, name, data_path, mode, params, test_full=False, mask_ind_file=None):
        ...
        self.no_data_norm = params['no_data_norm']
        self.vel_norm_norm = params['vel_norm_norm']

        self._get_normalize_stats()
    ...
    def _get_normalize_stats(self,):
        ...
        if self.no_data_norm:
            return

        if self.dynamic_len:
            if self.vel_norm_norm:
                self.vel_norm_max = max(np.linalg.norm(x, axis=-1).max() for x in train_vel)
            else:
                ...
                self.loc_max = max_loc
                self.loc_min = min_loc
                self.vel_max = max_vel
                self.vel_min = min_vel
        else:
            if self.vel_norm_norm:
                self.vel_norm_max = np.linalg.norm(train_vel, axis=-1).max()
            else:
    ...
    def _load_data(self, ):
        ...
        if self.dynamic_len:
            if self.vel_norm_norm:
                self.loc_feat = [feat / self.vel_norm_max for feat in self.loc_feat]
                self.vel_feat = [feat / self.vel_norm_max for feat in self.vel_feat]
            elif not self.no_data_norm:
            ...
            if self.vel_norm_norm:
                self.loc_feat = self.loc_feat / self.vel_norm_max
                self.vel_feat = self.vel_feat / self.vel_norm_max
            elif not self.no_data_norm:
