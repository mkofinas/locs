...
class SmallSynthData(Dataset):
    def __init__(self, data_path, mode, params):
        ...
        self.angles = torch.atan2(self.feats[..., 3], self.feats[..., 2])
        if not self.no_norm:
    ...
    def _normalize_data(self):
        ...
        if self.same_norm:
            ...
            self.vel_norm_max = np.linalg.norm(train_data[:, :, :, 2:], axis=-1).max()
            self.feats[:, :, :, :2] = self.feats[:, :, :, :2] / self.vel_norm_max
            self.feats[:, :, :, 2:] = self.feats[:, :, :, 2:] / self.vel_norm_max
        else:
            if self.symmetric_norm:
                self.loc_max = train_data[:, :, :, :2].abs().max()
                self.loc_min = -self.loc_max
                self.vel_max = train_data[:, :, :, 2:].abs().max()
    ...
    def unnormalize(self, data):
        if self.no_norm:
            ...
            result1 = data[:, :, :, :2] * self.vel_norm_max
            result2 = data[:, :, :, 2:] * self.vel_norm_max
